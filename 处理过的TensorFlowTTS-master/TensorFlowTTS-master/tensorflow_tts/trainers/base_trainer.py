...
class GanBasedTrainer(BasedTrainer):
    ...
    def create_checkpoint_manager(self, saved_path=None, max_to_keep=10):
        ...
        self.generator_ckpt = tf.train.Checkpoint(
            steps=tf.Variable(1), epochs=tf.Variable(1), optimizer=self.get_gen_optimizer()
        )
    ...
    def load_generator_checkpoint(self,pretrained_generator_path):
        """Load checkpoint."""
        # error
        self.generator_ckpt.restore(pretrained_generator_path)
        self.steps = self.generator_ckpt.steps.numpy()
        self.epochs = self.generator_ckpt.epochs.numpy()
        self._gen_optimizer = self.generator_ckpt.optimizer
    ...
    def fit(self, train_data_loader, valid_data_loader, saved_path, resume=None, pretrained_generator=None):
        ...
        with self._strategy.scope():
            ...
            if len(pretrained_generator) > 1:
                self.load_generator_checkpoint(pretrained_generator)
                logging.info(f"Successfully resumed from {pretrained_generator}.")
        self.run()
