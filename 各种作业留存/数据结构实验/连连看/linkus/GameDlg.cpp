...
BOOL CGameDlg::OnInitDialog()
{
	...
	{
	case 0:
		this->SetWindowText(_T("连连看基本模式"));
		break;
	case 1:
		this->SetWindowText(_T("连连看休闲模式"));
		break;
	case 2:
		this->SetWindowText(_T("连连看关卡模式"));
		break;
	default:
		break;
	}
	...
	//隐藏关卡显示
	GetDlgItem(IDC_LEVEL)->ShowWindow(SW_HIDE);
	m_nMax = 300;
	m_nStep = 1;

	//进度条设置
	m_pro.SetRange(0, m_nMax);
...
void CGameDlg::InitElement(void)
{
	...
	//加载 BMP 图片资源
	HANDLE hBmp = ::LoadImageW(NULL, elem,IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
void CGameDlg::UpdateMap(void)
{
	...
	int nElemH = size_Elem.cy;
	int nElemW = size_Elem.cx;
	int nLeft = m_ptGame.x;
	int nTop = m_ptGame.y;
	...
	for (int i = 0; i < nRows; i++)
	{
		for (int j = 0; j < nCols; j++)
		{
			...
			//先刷新背景（为后续重排准备）
			m_dcMem.BitBlt(nLeft + j * nElemW, nTop + i * nElemH, nElemW, nElemH,&m_dcBG, 0, 0, WHITENESS);
...
void CGameDlg::DrawTipLine(Vertex asvPath[4], int nVexnum) {
	...
	ndc.MoveTo(m_ptGame.x + asvPath[0].col * size_Elem.cx + size_Elem.cx / 2,
		m_ptGame.y + asvPath[0].row * size_Elem.cy + size_Elem.cy / 2);
	for (int i = 1; i < nVexnum; i++)
	{
		ndc.LineTo(m_ptGame.x + asvPath[i].col * size_Elem.cx + size_Elem.cx / 2,
			m_ptGame.y + asvPath[i].row * size_Elem.cy + size_Elem.cy / 2);
	}
...
void CGameDlg::DrawTipFrame(int nRow, int nCol) {
	...
	int left = m_ptGame.x + nCol*size_Elem.cx;
	int top = m_ptGame.y + nRow*size_Elem.cy;
	int right = left + size_Elem.cx;
	int bottom = top + size_Elem.cy;
...
void CGameDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (point.y < m_rtGameRect.top || point.y > m_rtGameRect.bottom || point.x < m_rtGameRect.left || point.x > m_rtGameRect.right)
	{
		...
		int nCol = (point.x - m_ptGame.x) / size_Elem.cx;
		int nRow = (point.y - m_ptGame.y) / size_Elem.cy;
		if (m_bFirstPoint) // 第一个点
		{
			m_GameC.SetFirstPoint(nRow,nCol);
			if (m_GameC.GetElement(nRow, nCol) != BLANK) DrawTipFrame(nRow, nCol);
		}
		else if(m_GameC.GetElement(nRow,nCol)!=BLANK)
		{
