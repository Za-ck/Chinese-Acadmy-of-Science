...
<form id="riskEventQuery" name="riskEventQuery" method="post" action="riskQuery/RiskEventQueryAction" onSubmit="return doValidate('riskEventQuery')">
...
<div class="queryDiv">
        ...
        <table style="margin-top:5px;">
        	<tr>
        		<td style="padding:5px 20px;">录入部门名称：<s:select name="reIndep" theme="simple" list="alldepList" listValue="depName" listKey="depName" class="text" style="width:240px" onKeyPress="EnterPress(event)" onKeyDown="EnterPress()"></s:select></td>
        		...
        		<td style="padding:5px 20px;">
        			查询时间：<s:select name="choosedId" theme="simple" list="#{'0':'--请选择--（默认为录入时间）','1':'录入时间','2':'发布时间'}"  style="width:154px;" onKeyPress="EnterPress(event)" onKeyDown="EnterPress()"></s:select>
        		</td>
        		<td style="padding:5px 20px;">&nbsp;&nbsp;从：<input name="dateFrom" id="dateFrom" class="Wdate" style="width:140px;" value="<s:property value='dateFrom'/>" onKeyPress="EnterPress(event)" onKeyDown="EnterPress()" onFocus="WdatePicker({isShowOthers:true,dateFmt:'yyyy-MM-dd'})"/>
		...
		<input type="button" class="querybutton" name="buttonAll" id="buttonAll" value="全部事件（默认录入时间）" onClick="showAllInfo();" />
		<input type="button" class="querybutton" name="buttonExport" id="buttonExport" value="导出事件" onClick="exportInfo();" />
		<!-- <input type="button" class="querybutton1" name="buttonExport1" id="buttonExport1" value="导出事件(发布时间)" onClick="exportInfo1();" /> -->
...
<div id='pagetable'> 
    <table id='mytable' class='tablestyle' width='100%' height='100%' >
    	<tr style="background-image:url(IconPage/images/thbg.png)">	
                		...
                		<td width="10%" align="center">时间<img src='IconPage/images/up.png' style='cursor:pointer;' id="ordersreDate" onClick="changeUpDown('rDate')" /></td>
                		<td width="11%" align="center">采取措施的时间计划<img src='IconPage/images/up.png' style='cursor:pointer;' id="ordersrmPlandate" onClick="changeUpDown('rmPlandate')" /></td>
		        		<td width="9%" align="center">综合评定<img src='IconPage/images/up.png' style='cursor:pointer;' id="ordersriAllvalue" onClick="changeUpDown('riAllvalue')" /></td>
            		...
            		<tr title="双击查看风险事件信息" align="center" onDblClick="handle('<s:property value='id.reId'/>','<s:property value='reEventname'/>')">
                      ...
                      <s:if test='reState=="*"'>
            			...
            			等待审核中...
            		 	...
            		 	<td align="center"><s:property value="rmDate"/></td>
				...
				<div id='pageDIV' style="background-image:url(IconPage/images/bg.png)">
				 ...
				 &nbsp;&nbsp;&nbsp;&nbsp;<img border='0' src='IconPage/images/line.gif'/>
&nbsp;&nbsp;&nbsp;&nbsp;<span>共有&nbsp;&nbsp;<%=request.getSession().getAttribute("pagecount")%>&nbsp;&nbsp;条记录,当前第&nbsp;&nbsp;<%=request.getAttribute("current_pagenum")%>/<%=pg%>页</span>
</div>          
...
{ 
	...
	if(confirm("确定撤回?"))
	{
		//document.forms[0].action='riskFlow/RERecallAction?current_pagenum=1&pageNum=10&recallPage=Q&id='+sum[countf].value;
...
function showCurrentYearInfo(){
	riskEventQuery.action="riskQuery/REQCurrentYearAction";
	riskEventQuery.submit();
}
//全部事件
function showAllInfo(){
	riskEventQuery.action="riskQuery/RiskEventQueryAction";
	riskEventQuery.submit();
}
//查询
function query()
{
	...
	riskEventQuery.action="riskQuery/REQQueryAction?current_pagenum=1"; 
	riskEventQuery.submit();
...
function reSetmess(){
	...
	document.getElementById('reIndep').value="--请选择--";
...
function prepage(){
    	...
    	riskEventQuery.action=s+current_page+"&pageNum=10";
    	riskEventQuery.submit();
}
//下一页
...
function firstpage(){
    	var s=document.getElementById("actionName").value+"?current_pagenum=";
    	riskEventQuery.action=s+1+"&pageNum=10";
    	riskEventQuery.submit();
}
//尾页
function lastpage(pageno){
		var s=document.getElementById("actionName").value+"?current_pagenum=";
    	riskEventQuery.action=s+pageno+"&pageNum=10";
    	riskEventQuery.submit();
}
//跳转
...
function changeUpDown(sorts){
	...
	if(path.src.indexOf('up.png')!=-1) { 
		...
		var s=document.getElementById("actionName").value+"?current_pagenum=1&pageNum=10&orderbys=";
		riskEventQuery.action=s+ordersorts;
		riskEventQuery.submit();
 	};
}
//导出数据
function exportInfo()
{
	...
	riskEventQuery.action="riskQuery/eventQueryExportAction";
	riskEventQuery.submit();
}
function exportInfo1()
{
	document.getElementById("buttonExport1").disabled=true;
	riskEventQuery.action="riskQuery/eventQueryExportAction1";
	riskEventQuery.submit();
}
//全选,取消
function allselect(){ 
