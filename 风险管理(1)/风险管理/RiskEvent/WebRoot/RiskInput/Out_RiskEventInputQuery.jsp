...
<div class="queryDiv">
		...
		<li style="float:left;margin-left:0px;font-size:14px;"> 录入部门名称：<s:select name="reIndep" theme="simple" list="#session.Out_alldepList" listValue="depName" listKey="depId" class="text" style="width:240px" onKeyPress="EnterPress(event)" onKeyDown="EnterPress()"></s:select>
</li>

        <li style="float:left;margin-left:20px;font-size:14px;"> 录入日期：<input name="dateFrom" id="dateFrom" class="Wdate" style="width:90px;" value="<s:property value='dateFrom'/>" onKeyPress="EnterPress(event)" onKeyDown="EnterPress()" onFocus="WdatePicker({isShowOthers:true,dateFmt:'yyyy-MM-dd'})"/>
</li>
...
<div id='pagetable'> 
    <table id='mytable' class='tablestyle' width='100%' height='100%' >
    	<tr style="background-image:url(IconPage/images/thbg.png)">				
                		...
                		<td width="13%" align="center">录入部门名称<img src='IconPage/images/up.png' style='cursor:pointer;' id="ordersdepartment.depName" onClick="changeUpDown('department.depName')" /></td>
            		...
            		<tr title="双击查看风险事件信息" align="center" onDblClick="window.location.href='riskInput/REIQReadAction?reId=<s:property value='reId'/>&backFlag=inputQuery'">
              			<td align="center" height="25"><input type="checkbox" name="idCheck"  value="<s:property value="reId"/>"/></td>
...
if(operation!=""){
	...
	else if(operation=="submitto"&&result=="success") alert("提交成功!");
	else if(operation=="submitto"&&result=="fail") alert("提交失败!");
...
//删除
function checkDelete(){ flag=false;var sum=document.getElementsByName("idCheck"); for(i = 0; i < sum.length; i++){ if(sum[i].checked == true){ flag = true;break;}} if(flag == false){ alert("请至少选择一条删除项!"); return false;} if(confirm("确定删除?")){document.forms[0].action='riskInput/REIQMultiDelAction?current_pagenum=1&pageNum=10';document.forms[0].submit(); return true;}}
//对字段排序 
var orderbycmd=0;
var ordersorts=""; 
...
function reSetmess()
{
   document.getElementById('dateFrom').value="";
	...
	document.getElementById('riskId').value="none1";
}
