 ...
 * $Id: validation-framework.js,v 1.3 2016/05/05 01:59:28 zxs Exp $
 */
...
// instead.
var ValidationRoot = "/RiskEvent/js/";
...
ValidationFramework.validateForm = function(fform) {
	...
	if (errorStrArray.length > 0) {
		...
		} else if(showError=="windows"){		
			if (showType == "first") {
			openBox('警告',350,150,0,errorStrArray[0],'');
			} else {
			openBox('警告',350,150,0,errorStrArray.join(br),'');
			}
...
ValidationFramework.validateDepend = function(field, depend) {
	...
	}	else if (depend.getName() == "officeTel") {
		return ValidateMethodFactory.validateOfficeTel(field, depend.getParams());
	}	else if (depend.getName() == "cellPhone") {
		return ValidateMethodFactory.validateCellPhone(field, depend.getParams());
	}
	else if(depend.getName()=="depNameField"){
		return ValidateMethodFactory.validateDepNameField(field,depend.getParams());
	}
...
ValidateMethodFactory.validateMask = function(field, params) {
	...
	if (obj == null) return false;
	if (obj.value == "") return true;
	var exp = new RegExp("^\\d{15}$|^\\d{17}(?:\\d|x|X)$");
	//FIXME: this method may be buggy, need more test.
	return exp.test(obj.value);
}
ValidateMethodFactory.validateOfficeTel = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if (obj.value == "") return true;
	var exp = new RegExp("^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})$|^\\d{4}$");
	//FIXME: this method may be buggy, need more test.
	return exp.test(obj.value);
}
ValidateMethodFactory.validateCellPhone = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if (obj.value == "") return true;
...
ValidateMethodFactory.validateDepNameField = function(field,params){//新增
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if (obj.value == "") return true;
	var exp = new RegExp("^[\u4E00-\u9FA5\uF900-\uFA2D\（\）]*$");
...
ValidationErrorString["zh-cn"]["double"]="{0}必须是一个整数或小数。";
...
ValidationErrorString["zh-cn"]["doublerange"]="{0}必须在小数{1}和{2}之间。";
ValidationErrorString["zh-cn"]["pid"]="{0}不是一个有效的身份证号。";
ValidationErrorString["zh-cn"]["officetel"]="{0}不是一个有效的办公电话。";
ValidationErrorString["zh-cn"]["cellphone"]="{0}不是一个有效的手机号码。";
...
ValidationErrorString["zh-cn"]["officeTel"]="{0} is not an valid officeTel.";
ValidationErrorString["zh-cn"]["cellPhone"]="{0} is not an valid cellPhone.";
