...
var count = 1;		//打开的窗口数目
var pretabId = "tab_id_index1";		//上一个打开的窗口id,默认是待办已办的窗口
...
function TabView(option) {
	...
	var tabTemplate = '<table class="tab_item" id="{0}" border="0" cellpadding="0" cellspacing="0" title="双击关闭所有窗口"><tr>' + '<td class="tab_item1"></td>'
	...
	var page = '<iframe id="{0}" frameborder="no" border="0" width="100%" height="100%" src="{1}"></iframe>';
	...
	function close(id) {
		...
		if ($(tab_context.current).attr("id") == close_tab.attr("id")) {
			...
			if(pretabId) {
				activate(pretabId, true);
				if(pretabId == id || pretabId == "tab_id_index1") {
					pretabId = null;
				}
				
			}
			else if (next.attr("id")) {
	...
	this.closeCurrentTab = function() {
		...
		var currentId = $(tab_context.current).attr("id");
		
		
		var close_page = $("#page_" + currentId);
		var close_tab = $("#" + currentId);
	...
	this.closeOtherTab = function(id) {
		
		this.tabs = this.tabContainer.find(".tab_item" + bottom);
		this.tabs.each( function() {
			var tabId = $(this).attr("id");
			if(tabId !== "tab_id_index1") {
				close(tabId);
			}
		});
		count = 1;
		activate("tab_id_index1",true);
	};
	...
	function getTabTitle(el) {
		...
		var theTab = $(el);
		var tab_title_obj = $(theTab).find(".tab_title");
		var tab_title = $(tab_title_obj).text();
		return tab_title;
	...
	this.add = function(option) {
		...
		this.tabs = this.tabContainer.find(".tab_item" + bottom);
		this.tabs.each( function() {
			var tab_title = getTabTitle(this);
			//alert(tab_title + "1");
			if($.trim(op1.title).indexOf($.trim(tab_title)) >= 0) {
				
				activate($(this).attr("id"),true);
				flag = true;
				return;
			}
		});
		...
		pretabId = $(tab_context.current).attr("id");
