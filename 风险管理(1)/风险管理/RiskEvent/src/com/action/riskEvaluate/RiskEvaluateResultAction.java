...
public class RiskEvaluateResultAction {
	...
	public String getEvaluateResult(){
		...
		investmentProject.setIpMark("国际投资项目");
...
//		this.investmentProjectDao.save(investmentProject);
		...
		for(int i = 0;i<elementCount.length;i++){
			...
			for(int j = 0;j<Integer.parseInt(elementCount[i]);j++){
				if(request.getParameter("text_10"+i+"00"+j)!=null){
					ProjectElement projectElement = new ProjectElement();
					projectElement.setPeProjectid(projectId);
					projectElement.setPeEvaluateid(request.getParameter("Id_10"+i+"00"+j));
					projectElement.setPeProbablity(request.getParameter("select_10"+i+"00"+j));
					
					if(request.getParameter("select_10"+i+"00"+j).equals("1")){
						projectElement.setPeProdegree("1（很小）");
					}
					else if(request.getParameter("select_10"+i+"00"+j).equals("2")){
						projectElement.setPeProdegree("2（较小）");
					}
					else if(request.getParameter("select_10"+i+"00"+j).equals("3")){
						projectElement.setPeProdegree("3（一般）");
					}
					else if(request.getParameter("select_10"+i+"00"+j).equals("4")){
						projectElement.setPeProdegree("4（较大）");
					}
					else if(request.getParameter("select_10"+i+"00"+j).equals("5")){
						projectElement.setPeProdegree("5（很大）");
					}
					projectElement.setPeImpactdegree(request.getParameter("text_10"+i+"00"+j));
					projectElement.setPeCateimpact(request.getParameter("text_10"+i));
					projectElement.setPeCategory(request.getParameter("category_10"+i+"00"+j));
					projectElement.setPeEvaluatename(request.getParameter("Idname_10"+i+"00"+j));
					projectElement.setPeCategoryname(request.getParameter("categoryname_10"+i+"00"+j));					
...
//					this.projectElementDao.save(projectElement);
			...
			elementSum.setPeCategoryname(request.getParameter("categoryname_10"+i+"000")+" ");//一级风险名称
			elementSum.setPeCateimpact(request.getParameter("text_10"+i)+" ");
	   ...
	   ItemLabelAnchor.OUTSIDE12, TextAnchor.BASELINE_CENTER)); 
		...
		rangeAxis.setMinorTickCount(2);
  	   ...
  	   try {
	       ...
	       boolean havaProjectByuserId=this.getInvestmentProjectDao().haveProjectByUserID(investmentProject.getIpName(),userId);
...
////////////////////////////////////////////////////////////////////////////////////////结束柱状图
	...
	public String RiskEvaluateGJQuery(){
		...
		tempList=this.getInvestmentProjectDao().findAllGJOrderByName(condition);
		getInfo(tempList);
		return "success";
	}
	
	public String RiskEvaluateGJQueryByCondition(){
		this.setIpName("请选择");
...
////////////////////////////////////////////////////////////////////////////////////////结束柱状图
	...
	public String updateEvaluateResult(){
		...
		String peTableList=request.getParameter("peTableList");
		String[] plist=peTableList.split("@");
		for (String pe : plist) {
			String[] peE=pe.split(";");
			projectElement=this.getProjectElementDao().findById(Integer.parseInt(peE[5]));
//			System.out.println("pe[5]**********"+peE[5]);
//			System.out.println("pe[5]**********"+peE[1]);
			projectElement.setPeCateimpact(peE[1]);
			projectElement.setPeProbablity(peE[3].substring(0,1));
			if(projectElement.getPeProbablity().equals("1")){
				projectElement.setPeProdegree("1（很小）");
			}
			else if(projectElement.getPeProbablity().equals("2")){
				projectElement.setPeProdegree("2（较小）");
			}
			else if(projectElement.getPeProbablity().equals("3")){
				projectElement.setPeProdegree("3（一般）");
			}
			else if(projectElement.getPeProbablity().equals("4")){
				projectElement.setPeProdegree("4（较大）");
			}
			else if(projectElement.getPeProbablity().equals("5")){
				projectElement.setPeProdegree("5（很大）");
			}
			projectElement.setPeImpactdegree(peE[4]);
			this.getProjectElementDao().updateProjectElement(projectElement);
...
//            System.out.println("getPeCategoryname"+"***********"+m+"*******"+peList.get(m).getPeCategoryname());
		...
		RiskEvaluate riskEvaluate = new RiskEvaluate();
		String[] finalResult = riskEvaluate.monteCarlo(listArray);
		investmentProject=this.getInvestmentProjectDao().findById(projectId);
		//结果存入数据库
        ...
        investmentProject.setIpRegion9(finalResult[10]);
        investmentProject.setIpRegion10(finalResult[11]);
//        this.getInvestmentProjectDao().updateInvestmentProject(investmentProject);
		...
		rangeAxis.setMinorTickCount(2);
  	   ...
  	   try {
	       ...
	       String userId=us.getUserId();
			...
			File[] files = folder.listFiles();
			for (File file : files) {
				if (file.getName().contains(userId +projectId)) {
					file.delete();
				}
			}
			Date dt = new Date();
			SimpleDateFormat df2 = new SimpleDateFormat(
					"yyyy-MM-dd-HH-mm-ss");
		       graphURL = "upload/" + userId+projectId+df2.format(dt)+".png"; 
		       filename = ServletActionContext.getServletContext().getRealPath(graphURL);
		       ChartUtilities. saveChartAsPNG( new File(filename), chart, 1000, 500);
  	   } catch (IOException e) {
  		   e.printStackTrace();
...
////////////////////////////////////////////////////////////////////////////////////////结束柱状图
	...
	public String selectProjectName(){
		projectNameList=this.getInvestmentProjectNameDAO().findAllGJ((current_pagenum-1)*pageNum,pageNum,projectNameString);
...
//			System.out.println("pName********"+pName);
			String isAdd="是";
			isAdd=this.getInvestmentProjectNameDAO().insertProjectName(pName);
//			System.out.println("isAdd***********"+isAdd);
	...
	public void addProjectNameContract() {
		try {
//			System.out.println("addProjectName*********");
			String pName=(String)ServletActionContext.getRequest().getParameter("pName");
//			System.out.println("pName********"+pName);
